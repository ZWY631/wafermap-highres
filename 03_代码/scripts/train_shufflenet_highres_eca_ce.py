#!/usr/bin/env python3
"""Train high-resolution ShuffleNetV2-ECA with Cross Entropy."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Train the high-resolution ShuffleNetV2-ECA "
    ...
    parser.add_argument(
        ...
        default=RANDOM_SEED,
        help=(
            "Random seed. Non-default seeds are saved "
            "under a seed-specific run name."
        ),
    )
    parser.add_argument(
        "--preflight",
        action="store_true",
        help=(
            "Validate output paths and exit without "
            "training or creating files."
        ),
    )
...
RUN_NAME = (
    BASE_RUN_NAME
    if EXPERIMENT_SEED == RANDOM_SEED
    else f"{BASE_RUN_NAME}_seed{EXPERIMENT_SEED}"
)
...
def main():
    ...
    log_lines = [
        "WM-811K HighRes ShuffleNetV2-ECA Cross Entropy training",
        f"run_name={RUN_NAME}",
        f"model={MODEL_NAME}",
        f"device={device}",
