#!/usr/bin/env python3
"""Evaluate the high-resolution ShuffleNetV2-ECA model."""
...
def load_best_model(device):
    ...
    if checkpoint.get("run_name") != RUN_NAME:
        raise ValueError(
            "Checkpoint run name does not match evaluator."
        )

    if checkpoint.get("model_name") != MODEL_NAME:
        raise ValueError(
            "Checkpoint model name does not match evaluator."
        )

    if checkpoint.get("focal_gamma") != FOCAL_GAMMA:
        raise ValueError(
            "Checkpoint focal gamma does not match evaluator."
        )

    model = ShuffleNetV2HighResECA()
...
def plot_confusion_matrix(matrix):
    ...
    axis.set_title(
        "High-Resolution ShuffleNetV2-ECA: Test Confusion Matrix"
    )
